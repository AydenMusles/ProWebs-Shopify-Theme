...
theme.init = function () {
  ...
  if( $('.template-product').length > 0 ) {
    if( $('.product-slider-holder').length > 0 ) {
      theme.slick();
    } else {
      theme.productSticky();
    }
...
theme.sections = function() {
  $(document).on('shopify:section:load', function(e) {
      ...
      if( $(container).hasClass('section-site-footer')) {
        theme.cacheSelectors();
        theme.header();
        theme.accessibleNav();
        theme.setupFocus();
      ...
      if( $(container).hasClass('section-store')) {
        theme.cacheSelectors();
        theme.initProducts();
        if( $('body').hasClass('ajax-on') ) {
          theme.ajaxCart();
      ...
      if( $(container).hasClass('section-article')) {
        theme.cacheSelectors();
        theme.share();
        theme.article();
        theme.initProducts();
    ...
    switch(id) {
      ...
      case "shopify-section-header":
        ...
        if( $('body').hasClass('no-scroll') ) {
          $('.js--close-drawer').trigger('click');
        }
        $('.drawer__close').removeClass('is-transitioning');
        */
        break;
    }
...
theme.variants = function() {
  ...
  $(document).on('change', '.single-option-selector', function() {
    ...
    var options = [];
    $(product).find('.single-option-selector:visible').each(function() {
...
theme.initProducts = function() {
    theme.variants();

  $('.section-product').each(function() {

    var productSection = $(this);
...
theme.productPage = function (options) {
  ...
  var $product_section = options.product_section,
      moneyFormat = options.money_format,
      variant = options.variant,
      selector = options.selector,
      singleProduct = options.single;


  //selectors
  var $productImage = $product_section.find('.ProductPhotoImg'),
      $addToCart = $product_section.find('.addToCart'),
      $productPrice = $product_section.find('.ProductPrice'),
      $comparePrice = $product_section.find('.ComparePrice'),
      $quantityElements = $product_section.find('.quantity-selector, label + .js-qty, .js-qty'),
      $addToCartText = $product_section.find('.AddToCartText');


  if (variant) {
    // Send slick slider to the variant image or update variant image if single
    if (variant.featured_image) {
      if(singleProduct) {
        var newImg = variant.featured_image,
            el = $productImage[0];
        newImgLarge = theme.Image.getSizedImageUrl(newImg.src,'1024x1024');
        newImgSmall = theme.Image.getSizedImageUrl(newImg.src,'50x50');

        $productImage.find('source').remove();
        var newSource = '<source data-srcset="' + newImgLarge + '" />';
        $productImage.append(newSource);
        var currentHeight = $productImage.find('img').outerHeight();
        ...
        $productImage.find('img').css('min-height', currentHeight + 'px');
      } else {
        var target = ".variant-" + variant.id;
        var index = "" + $(target).data('slick-index') + "";
        if($('.product-images-slider').length > 0) {
          theme.$productImagesSlider.slick('slickGoTo', index);
        }
      }
    }
    ...
    if (variant.available) {
      ...
      $addToCartText.html(theme.data.addToCartText);
      $quantityElements.show();
    } else {
      // Sold out, disable the submit button, change text, hide quantity elements
      $addToCart.addClass('disabled').prop('disabled', true);
      $addToCartText.html(theme.data.addToCartTextSoldOut);
    ...
    $addToCart.addClass('disabled').prop('disabled', true);
    $addToCartText.html(theme.data.addToCartTextUnavailable);
    $quantityElements.hide();
  }
};
...
theme.youtube = function() {
  ...
  $('.youtube').each(function() {
    $container = $(this);
    if($container.hasClass('youtube--uploads')) {
      theme.youtubeUploads($container);
    }
    if($container.hasClass('youtube--playlist')) {
      var $playlistid = $container.data('playlistid');
...
theme.youtubeUploads = function($container) {
...
theme.instagramInit = function() {
  $('.section--instagram').each(function() {
    if( $(this).hasClass('instagram--onboarding')) {
      return;
    }
    $accessToken = $(this).data('accesstoken');
    $noPosts = $(this).data('noposts');
    theme.instagram($accessToken, $(this), $noPosts);
  });
};

theme.instagram = function(accesstoken, item, noPosts) {
...
theme.slick = function() {
  ...
  theme.$productImagesSlider = $('.product-images-slider');

  theme.$productImagesSlider.on('init reInit', function(event, slick){
    $('.slick-prev').text("");
  ...
  theme.$productImagesSlider.on('init reInit afterChange', function(event, slick, currentSlide, nextSlide){
  ...
  theme.$productImagesSlider.slick({
    dots: true
  });
...
theme.productSticky = function() {

  var waypoints = $('.product-sticky').waypoint({
  ...
  var productWaypoint = $('.section-product').waypoint({
    handler: function(direction) {
      if(direction == "down" && $('.product-sticky').hasClass('fixed') ) {
        $('.product-sticky').addClass('fixed--bottom');
      }
      if(direction == "up") {
        $('.product-sticky').removeClass('fixed--bottom');
      }
    },
    offset: function() {
      var height = $('.section-product').height();
      var stickyHeight = $('.product-sticky').height();
      var offset = height - (stickyHeight - 48);
      return -(offset);
    }
  });
  ...
  function hideSticky() {
    $('.product-sticky').addClass("fade");
  }

  if( $('.product-images-slider__image--single').length > 0 ) {
    $('.product-images-slider__image--single').load(function() {
      $('.product-sticky').removeClass("fade fixed fixed--bottom");
      Waypoint.refreshAll();
    });
  }
  ...
  window.addEventListener("resize", hideSticky);
  window.addEventListener("resize", updateLayout, false);


};

theme.accountHover = function() {
  var $account = $('.login');
  var $nextIcon;
...
theme.article = function() {
  $('.rte--article p').each(function(i){          // For each paragraph
...
theme.page = function() {
  $('.rte--page p').each(function(i){          // For each paragraph
      if ( ($(this).find('img').length) &&     // If there's an image
           (!$.trim($(this).text()).length))   // and there's no text
      {
          $(this).addClass('has-image');         // Add a special CSS class
